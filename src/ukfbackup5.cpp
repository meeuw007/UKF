...
void UKF::Prediction(double delta_t) {
	...
	*/
	...
	MatrixXd sigma_points = MatrixXd(5, 11);
	...
	double lambda_scaling = std::sqrt(lambda_ + n_x_);

	for (int index = 0; index < n_x_; ++index)
	{
		sigma_points.col(index + 1) = x_ + (lambda_scaling * P_root.col(index));
		sigma_points.col(index + n_x_ + 1) = x_ - (lambda_scaling * P_root.col(index));

	}

	/////Create augmented_sigma_points matrix
	...
	double lambda_scalings = std::sqrt(lambda_ + n_aug_);

	for (int index = 0; index < n_aug_; ++index)
	{
		augmented_sigma_points.col(index + 1) =
			x_augmented + (lambda_scalings * P_augmented_root.col(index));

		augmented_sigma_points.col(index + n_aug_ + 1) =
			x_augmented - (lambda_scalings * P_augmented_root.col(index));
	}
	/////augmented_sigma_point matrix created
	...
	for (int index = 0; index < 2 * n_aug_ + 1; ++index)
	{
		...
		VectorXd transition_vector = VectorXd(n_x_);

		if (std::abs(yaw_speed) < 0.001)
		{
			transition_vector(0) = delta_t * longitudinal_speed * std::cos(yaw);
			transition_vector(1) = delta_t * longitudinal_speed * std::sin(yaw);
		...
		transition_vector(3) = yaw_speed * delta_t;
		transition_vector(4) = 0;

		VectorXd noise_vector = VectorXd(n_x_);
		double half_squared_time_delta = 0.5 * delta_t * delta_t;
	...
	for (int index = 0; index < 2 * n_aug_ + 1; ++index)
	{
		VectorXd difference = Xsig_pred_.col(index) - x_;

		// Normalize yaw angle 
		while (difference(3) < -M_PI) { difference(3) += 2.0 * M_PI; }
		while (difference(3) >  M_PI) { difference(3) -= 2.0 * M_PI; }

		covariance_matrix += weights_(index) * difference * difference.transpose();
	}
...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
	...
	*/
	...
	for (int index = 0; index < 2 * n_aug_ + 1; ++index)
	{
		VectorXd difference = measurements_predictions.col(index) - mean_measurement_prediction;
		S += weights_(index) * difference * difference.transpose();
	}
	...
	for (int index = 0; index < 2 * n_aug_ + 1; ++index)
	{
		...
		VectorXd state_difference = Xsig_pred_.col(index) - x_;
		VectorXd measurement_difference = measurements_predictions.col(index) - mean_measurement_prediction;

		T += weights_(index) * state_difference * measurement_difference.transpose();
	...
	// Update
	x_ = x_ + (K * measurement_difference);
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
	...
	*/
	...
	for (int index = 0; index < 2 * n_aug_ + 1; ++index)
	{
		VectorXd difference = measurements_predictions.col(index) - mean_measurement_prediction;
		//normalize angle
		while (difference(1) < -M_PI) { difference(1) += 2.0 * M_PI; }
		while (difference(1) >  M_PI) { difference(1) -= 2.0 * M_PI; }
	...
	for (int index = 0; index < 2 * n_aug_ + 1; ++index)
	{
		...
		VectorXd state_difference = Xsig_pred_.col(index) - x_;
		//normalize angle
		while (state_difference(3) < -M_PI) { state_difference(3) += 2.0 * M_PI; }
		while (state_difference(3) >  M_PI) { state_difference(3) -= 2.0 * M_PI; }

		VectorXd measurement_difference = measurements_predictions.col(index) - mean_measurement_prediction;
		//normalize angle
		while (measurement_difference(1) < -M_PI) { measurement_difference(1) += 2.0 * M_PI; }
		while (measurement_difference(1) >  M_PI) { measurement_difference(1) -= 2.0 * M_PI; }
